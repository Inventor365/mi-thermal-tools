# 🔥 Mi Thermal Editor (Cross-Platform Edition)
...
### Quick Setup (1-Click Installer)

**Windows (PowerShell 5.1+):**
```powershell
iwr -useb https://raw.githubusercontent.com/Inventor365/mi-thermal-tools/main/scripts/install.ps1 | iex
```

**Linux / macOS (Bash):**
```bash
curl -fsSL https://raw.githubusercontent.com/Inventor365/mi-thermal-tools/main/scripts/install.sh | bash
```
*Note: This will install Python dependencies and place a shortcut right on your Desktop.*
...
### 1. Native Desktop GUI (PySide6)

Launch the native desktop interface:
```bash
# Launch generic shortcut or via cli
mi-thermal-editor gui
```

Features included in the Desktop GUI:
- **File Explorer Sidebar**: Intelligent recursive folder parsing, one-click presets for `/odm/etc` and `/vendor/etc`, live search filter, and encryption status badges (`🔒 Encrypted` / `📄 Plaintext`).
- **Code Editor**: Real-time syntax highlighting for thermal configuration syntax, light/dark responsive, explicit save / export dialogs.
- **Visual Analyzer Tab**: Tabulated sensor threshold curves and device throttling action lists mapped to SCONFIG variants.
- **Diff & Compare Tab**: Visual side-by-side section and unified line diff.
- **Batch Processing Tab**: Fast, threaded background recursive decryption and encryption.
...
## 🛠️ï¸ Deep-Dive Thermal Configuration Anatomy
...
├── mi_thermal_editor/
│   ├── core/                     # Core crypto, parsing, analyzer logic
│   ├── services/                 # ADB and external device services
│   ├── ui/                       # Modern PySide6 Qt Desktop UI components
